        ...
        const void *params)
{
    ...
    bool have_omm_usage_counts = false;
    ...
    d3d12_command_list_check_render_pass_validation(list, "BuildRaytracingAccelerationStructureEx called within a render pass.\n", true);
    ...
    if (!vkd3d_acceleration_structure_convert_inputs_nv(list->device, &desc->inputs,
            build_info, geometry_infos, omm_infos, range_infos, primitive_counts, &have_omm_usage_counts))
    {
                ...
                for (i = 0; i < desc->inputs.numDescs; i++)
                {
                    ...
                    switch (geom->type)
                    {
                        ...
                        case NVAPI_D3D12_RAYTRACING_GEOMETRY_TYPE_OMM_TRIANGLES_EX:
                            VKD3D_BREADCRUMB_TAG("OMM Triangle [Flags, VBO VA, VBO stride, IBO, Transform, VBO format, IBO format, V count, I count, OMM IBO VA, OMM IBO stride, OMM IBO format, OMM base location, OMM array, OMM usage counts]");
                            VKD3D_BREADCRUMB_AUX32(geom->flags);
                        ...
                        default:
                            VKD3D_BREADCRUMB_TAG("Unsupported Geometry Type");
                            VKD3D_BREADCRUMB_AUX32(geom->type);
                            break;
                    }
                }
            }
        ...
        void *params)
{
    ...
    d3d12_command_list_check_render_pass_validation(list, "BuildRaytracingOpacityMicromapArray called within a render pass.\n", true);
    d3d12_command_list_flush_dgc_batch(list);
    ...
    if (rtas_batch->build_info_count)
    {
        ...
        vk_barrier.dstStageMask = VK_PIPELINE_STAGE_2_ACCELERATION_STRUCTURE_BUILD_BIT_KHR;
        vk_barrier.dstAccessMask = VK_ACCESS_2_ACCELERATION_STRUCTURE_READ_BIT_KHR |
    ...
    rtas_batch->build_type = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_OPACITY_MICROMAP_ARRAY;

    desc = nvParams->pDesc;

    usage_info_count = desc->inputs.numOMMUsageCounts;

    if (!d3d12_command_list_allocate_omm_build_info(list, usage_info_count,
            &build_info, &usage_infos))
        return NVAPI_OUT_OF_MEMORY;

    if (!vkd3d_opacity_micromap_convert_inputs_nv(list->device, &nvParams->pDesc->inputs, build_info, usage_infos))
    {
        ERR("Failed to convert inputs.\n");
        return NVAPI_ERROR;
    }
    ...
    build_info->scratchData.deviceAddress = desc->scratchOpacityMicromapArrayData;
        ...
        const void *params)
{
    ...
    d3d12_command_list_check_render_pass_validation(list, "EmitRaytracingOpacityMicromapArrayPostbuildInfo called within a render pass.\n", true);
    d3d12_command_list_flush_dgc_batch(list);
        ...
        const void *params)
{
    ...
    d3d12_command_list_check_render_pass_validation(list, "RaytracingExecuteMultiIndirectClusterOperation called within a render pass.\n", true);
    d3d12_command_list_flush_dgc_batch(list);
